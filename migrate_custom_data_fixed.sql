-- Fixed migration script to import custom transaction data into Firefly III
...
SET @user_group_id = (SELECT id FROM user_groups WHERE title = 'Default Group' LIMIT 1);

-- Create default accounts if they don't exist
INSERT IGNORE INTO accounts (
    created_at, updated_at, user_id, user_group_id, account_type_id, name, 
    virtual_balance, iban, active, encrypted, `order`
) VALUES 
(NOW(), NOW(), 1, @user_group_id, 3, 'Main Checking Account', 0.00, NULL, 1, 0, 1),
(NOW(), NOW(), 1, @user_group_id, 4, 'General Expenses', 0.00, NULL, 1, 0, 2);

-- Set variables for account IDs
SET @asset_account_id = (SELECT id FROM accounts WHERE user_id = 1 AND account_type_id = 3 AND name = 'Main Checking Account' LIMIT 1);
SET @expense_account_id = (SELECT id FROM accounts WHERE user_id = 1 AND account_type_id = 4 AND name = 'General Expenses' LIMIT 1);
SET @withdrawal_type_id = 1; -- Withdrawal transaction type
SET @eur_currency_id = 1;
SET @chf_currency_id = 27;
SET @usd_currency_id = 12;

-- Create transaction groups for each custom transaction
INSERT INTO transaction_groups (created_at, updated_at, user_id, title)
SELECT 
    t.created_at,
    t.updated_at,
    1 as user_id,
    CONCAT('Transaction #', t.id, ' - ', t.shop, ' on ', DATE(t.date)) as title
...
-- Withdrawal from asset account (negative amount)
...
-- Credit to expense account (positive amount)
INSERT INTO transactions (
    created_at, updated_at, transaction_journal_id, account_id, amount, 
    foreign_amount, foreign_currency_id, description, identifier
)
